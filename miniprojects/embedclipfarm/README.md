...
A proxy worker for fetching YouTube data from the browser. Deploy to [Cloudflare Workers](https://workers.cloudflare.com) (free tier):

1. Create a new Worker at workers.cloudflare.com
2. Paste the contents of `worker.js` and deploy
3. Use the worker URL in the web UI settings

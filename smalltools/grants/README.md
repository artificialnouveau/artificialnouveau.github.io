...
- **`region`** — Must be exactly one of: `EU`, `US`, `UK`, `NL`, `Asia`, `Africa`, `Canada`, `Australia`, `LatAm`, `Remote`, `Worldwide`. Drives the Region filter and the static-page generation. If you need a new region, see "Adding a new region" below. Special case: Switzerland is a tag-driven sub-region. Give Swiss calls `region: "EU"` plus a `switzerland` tag; they then appear under BOTH the EU and Switzerland filters, feeds, calendars and SEO pages (see `region_matches` in `generate_feed.py` and `grantRegions` in `index.html`). Never set `region: "Switzerland"` directly.
...
- **`tags`** — Free-text tags for search. Lowercase, hyphen-separated. The card's coloured tab label is NOT taken from tags anymore; it always shows the opportunity type (RESIDENCY, FELLOWSHIP, PRIZE, GRANT, FESTIVAL, JOB, or OPEN CALL), detected by `detectType()` in `index.html` from the tags and title. Include a `residency`/`fellowship`/`prize`/`grant` tag where applicable so the type detection and the Type filter classify the entry correctly.
